...
void NetEngine::update(int timeoutMilliseconds) {
	...
	int pollRet = Poll(_pollfds.data(), _pollfds.size(), timeoutMilliseconds);
	if (pollRet < 0) {
		SITA_ASSERT(false);
		return;
	}
...
void NetEngine::_addToPoll(Vector<UPtr<NESocket>>& list) {
	...
	while (it != list.end()) {
		...
		if (!s || !s->_sock.isValid() || s->_status == NESocket::Status::None) {
			it = list.erase(it); // destroy closed sockets;0
			//TODO erase_unsorted
			continue;
		}
...
void NetEngine::onRecv(NESocket* s) {
	...
	for (size_t i = 0; i < _maxPacketPerFrame; i++) {
		...
		{
			BinDeserializer se(_recvPacketBuf.data(), _recvPacketBuf.size());
			se.io_fixed(hdr.len);
			hdr.len ^= _lenKey;
		}
