...
public:
	...
	int sendto(const SockAddr& addr,const u8* data, size_t dataSize);
	int sendto(const SockAddr& addr, StrView  data)		{ return sendto(addr, reinterpret_cast<const u8*>(data.data()), data.size()); }

	int send(const u8* data, size_t dataSize);
	int send(const StrView  data)						{ return send(reinterpret_cast<const u8*>(data.data()), data.size()); }
